...
lock_dir="${TMPDIR:-/tmp}/aerospace-ghostty-tabs-fix.lock"
...
if [[ "$focused_bundle_id" != "com.mitchellh.ghostty" ]]; then
  exit 0
fi
...
ghostty_count="$("$AEROSPACE_BIN" list-windows --workspace "$workspace" --app-bundle-id com.mitchellh.ghostty --count 2>/dev/null || echo 0)"
total_count="$("$AEROSPACE_BIN" list-windows --workspace "$workspace" --count 2>/dev/null || echo 0)"

# Only auto-switch when the workspace is "just Ghostty". This avoids turning a
# mixed-app workspace into a global accordion layout unexpectedly.
if [[ "$ghostty_count" != "$total_count" ]]; then
  exit 0
fi

# Ghostty "tabs" on macOS surface as multiple windows. In a tiling layout, that
# creates a blank half because only the focused tab-window draws. Switching to
# accordion stacks them like tabs.
# Keep the workspace in accordion even when only one Ghostty window exists.
